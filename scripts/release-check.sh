...
grep -q 'On `PREVIEW_ACTION_REQUIRED` or `PREVIEW_FAILED`, no HTML exists' skills/md2wechat/SKILL.md || fail "skills/md2wechat/SKILL.md must describe action-required and failed preview no-file behavior"
...
grep -q 'On `PREVIEW_ACTION_REQUIRED` or `PREVIEW_FAILED`, no HTML exists' platforms/openclaw/md2wechat/SKILL.md || fail "OpenClaw skill must describe action-required and failed preview no-file behavior"
