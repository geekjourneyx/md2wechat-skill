...
grep -q 'with `--json`, inspect diagnostics are returned in `data.inspect`' skills/md2wechat/SKILL.md || fail "skills/md2wechat/SKILL.md must scope preview inspect diagnostics to JSON"
...
grep -q 'with `--json`, inspect diagnostics are returned in `data.inspect`' platforms/openclaw/md2wechat/SKILL.md || fail "OpenClaw skill must scope preview inspect diagnostics to JSON"
