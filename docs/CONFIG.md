...
如果你要从零配置、迁移多公众号、或让 Agent 按步骤排查配置，先看 [配置保姆级指南](CONFIG-WALKTHROUGH.md)。本文更偏参考手册，解释配置字段、优先级和高级选项。
...
当前内置 provider：`openai`、`minimax`、`atlascloud` (`atlas-cloud` / `atlas`)、`tuzi`、`modelscope` (`ms`)、`openrouter` (`or`)、`requesty` (`rq`)、`gemini` (`google`)、`volcengine` (`volc`)。
